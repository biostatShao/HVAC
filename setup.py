...
extensions = [
    Extension("hvac.utils.math_utils",
              ["hvac/utils/math_utils.pyx"],
              libraries=[[], ["m"]][os.name != 'nt'],  # Only include for non-Windows systems
              include_dirs=[np.get_include()],
              extra_compile_args=["-O3"]),
    Extension("hvac.model.vi.e_step",
              ["hvac/model/vi/e_step.pyx"],
              include_dirs=[np.get_include()],
              define_macros=[("NPY_NO_DEPRECATED_API", "NPY_1_7_API_VERSION")],
              extra_compile_args=["-O3"]),
    Extension("hvac.model.vi.e_step_cpp",
              ["hvac/model/vi/e_step_cpp.pyx"],
              language="c++",
...
if check_openmp_support():
    ...
    openmp_extensions = ['hvac.model.vi.e_step_cpp', 'hvac.model.vi.e_step']
...
setup(
    ...
    version="0.0.1",
    author="Zhonghe Shao",
    author_email="zhonghe@hust.edu.cn",
    description="Hierarchical Bayesian model with Variational inference and functional Annotation integration for Cross-ancestry prediction",
    url="https://github.com/biostatShao/HVAC",
    classifiers=[
